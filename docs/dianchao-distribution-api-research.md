...
>
> 配置纠正：真实 AppKey 为 `3432336`；此前针对错误 AppKey `3255489` 的控制台核验记录不能证明真实应用已经获权，需在首次 OAuth 联调时重新确认。
...
公开方案页面仍不能证明某个 AppKey 已实际获权；真实 AppKey `3432336` 必须在 OAuth 联调时重新完成带结果的验收表：
