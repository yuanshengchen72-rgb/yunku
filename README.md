...
`https://dianchao1688.com/`。

当前版本已实现微信小店绑定、铺货任务持久化和进程内后台执行器。任务会依次转存商品图片、自动匹配店铺可经营类目、读取运费模板并提交微信商品；应用重启后也会自动续跑遗留的待处理任务。真实发品仍要求客户店铺 AppID/AppSecret 有效、聚石塔固定出口 IP 可访问微信接口、店铺具备对应类目权限且已配置运费模板。外部条件不满足时，任务会明确标记失败并显示微信接口返回原因，不再长期停留在待处理。
